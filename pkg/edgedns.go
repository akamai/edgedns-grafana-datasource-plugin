...
// Edge DNS "authoritative-dns-traffic-by-time" OPEN API documentation
// https://developer.akamai.com/api/core_features/reporting/authoritative-dns-traffic-by-time.html

// Akamai OPEN EdgeGrid for GoLang v1
// https://github.com/akamai/AkamaiOPEN-edgegrid-golang/

const EDGEDNS_URL_FORMAT = "/reporting-api/v1/reports/authoritative-dns-traffic-by-time/versions/3/report-data?start=%v&end=%v&interval=%v"
...
// EdgeDns OPEN API insists that start and end times must be on interval boundaries.
...
func createOpenUrl(fromRounded time.Time, toRounded time.Time, interval Interval) string {
	return fmt.Sprintf(EDGEDNS_URL_FORMAT, openApiUrlTimeFormat(fromRounded), openApiUrlTimeFormat(toRounded), interval)
}
...
// {"objectType": "edns", "objectIds": ["abcdefghi.com", "akamai.thisismydomain.com"], "metrics": ["sum_hits", "sum_nxdomain"]}
// {"objectType": "edns", "objectIds": ["all"], "metrics": ["sum_hits", "sum_nxdomain"]}

// OPEN API request body contructor
func NewEdgeDnsTrafficByTimeReqDto(zoneNames []string) *EdgeDnsTrafficByTimeReqDto {
	return &EdgeDnsTrafficByTimeReqDto{
		ObjectType: "edns",
		ObjectIds:  zoneNames,
		Metrics:    []string{"sum_hits", "sum_nxdomain"},
	}
}

type EdgeDnsTrafficByTimeReqDto struct {
...
type Datum struct {
	...
	SumNxDomain   string `json:"sum_nxdomain"`
	SumRequests   string `json:"sum_requests"`
}
